...
distb_boule = 1000*ones(N);
r = 0.02;

for i=1:MM,
distb_boule =  min(distb_boule,sqrt( (X1 - 0.8*(rand(1,1)-0.5)).^2 + (X2 - 0.8*(rand(1,1)-0.5)).^2 + (X3 - 0.7*(rand(1,1)-0.5)).^2  ) - r);
end

U_domaine = 1/2 - tanh(0.5*distb_boule/(epsilon))/2;
   
  
  vv = VideoWriter(['Steiner_alea_',num2str(MM),'.avi'],'Motion JPEG AVI');
  vv.Quality = 90;
  open(vv);
...
disp(['step ',num2str(i),' / ',num2str(2*T/dt)])
temps(i) = i*dt;
U = U1;
...
coef = 0.5*0.75/2.5*epsilon*N(1)^3;
...
pause(0.1);    
  ...
  view(200,20)
       ...
       name_fig = ['Test_steiner_alea_',num2str(MM),'_',num2str( j_sauvegarde),'.eps'];
