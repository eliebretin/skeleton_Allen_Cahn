...
elseif (no_cond==4)
 ...
 L3 = N/16 - 2;
 %U = rand(N,N,N);
 %U(N/4+1:3*N/4,N/4+1:3*N/4,N/4+1:3*N/4)=ones(N/2,N/2,N/2);
 U(L1+1:N-L1,L2+1+2*2:2*2+N-L2,L3-2*2+1:N-L3-2*2)=ones(N-2*L1,N-2*L2,N-2*L3);
 ...
 %U = rand(N,N,N);
 %U(N/4+1:3*N/4,N/4+1:3*N/4,N/4+1:3*N/4)=ones(N/2,N/2,N/2);
 U(2:N-2*L1+1,2:N-2*L2+1,2:N-2*L3+1)=ones(N-2*L1,N-2*L2,N-2*L3);
 
X1 = (X1-0.25).^2;
X2 = (X2 +0).^2;
X3 = (X3+0).^2;
R0 = 0.2;
U1= (sqrt(1/3*X1 + X2 + X3) - 0.25)<0;
