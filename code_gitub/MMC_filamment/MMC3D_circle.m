...
x = linspace(-1/2,1/2,N(1));
 ...
 epsilon = 2/N(1);
    ...
    vv = VideoWriter('circle.avi','Motion JPEG AVI');
    vv.Quality = 90;
    open(vv);
 
 

pos1_function = @(theta)  0.4*cos(theta);
pos2_function = @(theta)  0.4*sin(theta);
pos3_function = @(theta)  0 + 0*(-1 + theta/(2*pi)*2)  ;
...
pause(0.1);
  ...
  %view(200,20)
       ...
       name_fig = ['Test_circle_',num2str( j_sauvegarde),'.eps'];
...
plot(temps,volU1.^2)
 name_title = ['t -> R^2 '];
title(name_title,'linewidth',2)
  name_fig = ['Test_circle_rayonplot.eps'];
